...
async function createComplaint(req, res) {
  ...
  if (global.__campusfixMongo) {
    ...
    await createNotification({
      ...
      message: `${ticketId} has been created.`,
  ...
  db.notifications.push({
    _id: `n_${Date.now()}_${Math.random().toString(16).slice(2)}`,
    userId: req.user.id,
    title: "Report submitted",
    message: `${ticketId} has been created and assigned to ${data.assignedDepartment}.`,
    complaintId: ticketId,
    read: false,
    createdAt: new Date(),
  });
...
async function mine(req, res) {
  if (global.__campusfixMongo)
    return res.json(
      await Complaint.find({ reporterId: req.user.id }).sort({ createdAt: -1 }),
    );
  res.json(
    allComplaints()
      .filter((c) => c.reporterId === req.user.id)
...
async function updateStatus(req, res) {
  ...
  if (global.__campusfixMongo) await item.save();
    ...
    const db = demoData();
    const i = db.complaints.findIndex((c) => c.ticketId === item.ticketId);
    db.complaints[i] = item;
    db.notifications.push({
      _id: `n_${Date.now()}`,
      userId: item.reporterId,
      title: "Complaint updated",
      message: `${item.ticketId}: ${status}`,
...
async function assign(req, res) {
  ...
  item.timeline.push({
    status: "Assigned",
    note: `Assigned to ${item.assignedDepartment}`,
    by: req.user.id,
    at: new Date(),
  });
  if (global.__campusfixMongo) await item.save();
    ...
    db.notifications.push({
      ...
      message: `${item.ticketId} assigned to ${item.assignedDepartment}.`,
      complaintId: item.ticketId,
      read: false,
      createdAt: new Date(),
    });
...
async function verify(req, res) {
  ...
  if (item.reporterId !== req.user.id)
    return res
      .status(403)
      .json({ message: "Only the reporter can verify this complaint" });
  item.studentVerified = true;
  item.status = "Resolved";
  item.timeline = item.timeline || [];
  item.timeline.push({
    status: "Resolved",
    note: "Student verified resolution",
    by: req.user.id,
    at: new Date(),
  });
  if (global.__campusfixMongo) await item.save();
    ...
    db.notifications.push({
      ...
      message: `${item.ticketId} has been verified as resolved.`,
      complaintId: item.ticketId,
      read: false,
      createdAt: new Date(),
    });
