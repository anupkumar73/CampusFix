...
const rules = [
  ...
  },
  {
    ...
    words: ["wifi", "wi-fi", "internet", "network", "router", "connectivity"],
  },
  {
  ...
  },
  {
    ...
    words: ["garbage", "trash", "dirty", "clean", "waste", "dust", "smell"],
  },
  {
...
function classify(text = "") {
  ...
  for (const rule of rules) {
    const score = rule.words.reduce(
      (n, word) => n + (normalized.includes(word) ? 1 : 0),
      0,
    );
  ...
  else if (highWords.some((w) => normalized.includes(w)))
